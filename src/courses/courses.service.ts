...
export class CourseService {
  ...
  public async update(id: string, updateCourseDto: UpdateCourseDto): Promise<void> {
    try {
      await this.findOne(id);
      const updateData = {
        ...updateCourseDto,
        modules: updateCourseDto.modules ? Promise.resolve(updateCourseDto.modules) : undefined,
        tags: updateCourseDto.tagIds ? Promise.resolve(updateCourseDto.tagIds.map(id => ({ id }))) : undefined
      };
