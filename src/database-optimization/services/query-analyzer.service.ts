...
export class QueryAnalyzerService {
  ...
  private async getExecutionPlan(query: string): Promise<QueryExecutionPlan> {
    const explainQuery = this.config.includeExplainAnalyze
  ...
  private generateOptimizationSuggestions(plan: QueryExecutionPlan, query: string): OptimizationSuggestion[] {
    ...
    if (this.hasExpensiveNestedLoop(plan)) {
      suggestions.push({
        ...
        implementation: "Add WHERE clauses to reduce dataset size or create indexes on join columns",
      })
    }
  ...
  private generateIndexRecommendations(plan: QueryExecutionPlan, query: string): IndexRecommendation[] {
  ...
  private updateQueryHistory(query: string, executionTime: number): void {
    ...
    if (existing) {
      ...
      existing.avgExecutionTime =
        (existing.avgExecutionTime * (existing.totalExecutions - 1) + executionTime) / existing.totalExecutions
  ...
  private isMissingWhereClause(query: string): boolean {
    ...
    return (
      ...
      (normalizedQuery.includes("select") || normalizedQuery.includes("update") || normalizedQuery.includes("delete"))
    )
  }
